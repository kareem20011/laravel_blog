    ...
    @endif
        <div class="container">
            ...
            <div class="card-bolck">
                <img class="edit_page" src="{{asset($post->image)}}" alt="">
                    ...
                    @foreach (config('app.languages') as $key=>$lang)
                        <div id="{{$key}}" class="tabcontent @if($loop->index == 0) activex @endif">
                            <div class="md:w-full p-3">
                                <input value="{{$post->translate($key)->title}}" type="text" name="{{$key}}[title]" placeholder="{{__('words.title')}}" class="block w-full p-4 text-gray-900 border border-gray-300 rounded-md bg-gray-50 sm:text-xs focus:ring-blue-500 focus:border-blue-500 my-5 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                <input value="{{$post->translate($key)->tags}}" placeholder="{{__('words.tags')}}" type="text" name="{{$key}}[tags]" class="block w-full p-4 text-gray-900 border border-gray-300 rounded-md bg-gray-50 sm:text-xs focus:ring-blue-500 focus:border-blue-500 my-5 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                <label for="editor">{{__('words.content')}}</label>
                                <textarea id="editor" name="{{$key}}[content]" class="block w-full p-4 text-gray-900 border border-gray-300 rounded-md bg-gray-50 sm:text-xs focus:ring-blue-500 focus:border-blue-500 my-5 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-blue-500 dark:focus:border-blue-500">
        ...
        <button class="btn btn-primary" style="color: black;" type="submit">{{__('words.editPost')}}</button>
    </div>
</form>
